    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        self.observation_space = spaces.Dict(
            {
                'lander_cartesian': spaces.Box(low[0:4], high[0:4]),
                'lander_angular': spaces.Box(low[4:6], high[4:6]),
                'legs_contact':  spaces.Box(low[6:8], high[6:8])
            }
        )
    ...
    def step(self, action):
        ...
        state = {
            'lander_cartesian': np.array(state[0:4]).astype(np.float32),
            'lander_angular': np.array(state[4:6]).astype(np.float32),
            'legs_contact': np.array(state[6:8]).astype(np.float32)
        }
    ...
    def get_observation_schema(self):
        return {
            'lander_cartesian':
                {
                    'description': 'x and y position of the lander and the linear velocities vx and vy',
                    'type': 'float',
                    'size': 4
                },
            'lander_angular':
                {
                    'description': 'lander angle and its angular velocity',
                    'type': 'float',
                    'size': 2,
                },
            'legs_contact':
                {
                    'description': 'booleans that represent whether each leg is in contact with the ground or not',
                    'type': 'bool',
                    'size': 2
                }
