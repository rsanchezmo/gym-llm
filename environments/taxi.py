    ...
    * v0: Initial version release
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.observation_space = spaces.Dict(
            {'state': spaces.Discrete(num_states)}
        )
    ...
    def step(self, a):
        ...
        if self.render_mode == "human":
            self.render()
        return {'state': int(s)}, r, t, False, {"prob": p, "action_mask": self.action_mask(s)}
    ...
    def reset(
        ...
        *,
        ...
        if self.render_mode == "human":
            self.render()
        return {'state': int(self.s)}, {"prob": 1.0, "action_mask": self.action_mask(self.s)}
    ...
    def get_observation_schema(self):
        return {
            'state':
                {
                    'description': """There are 500 discrete states since there are 25 taxi positions, 
                    5 possible locations of the passenger (including the case when the passenger is in the taxi), and 4 destination locations.
                    Destination on the map are represented with the first letter of the color. Lines in the map are obstacles.
                    Passenger locations:
                    0: Red
                    1: Green
                    2: Yellow
    ...
    def get_goal_description(self):
        ...
        The episode ends if the taxi drops off the passenger.
        The map is:
        +---------+
        |R: | : :G|
        | : | : : |
