...
class BlackjackEnv(gym.Env, LLMEnv):
    """
    ...
    * v0: Initial version release
    ...
    def __init__(self, render_mode: Optional[str] = None, natural=False, sab=False):
        ...
        self.observation_space = spaces.Dict(
            {
                "players_sum": spaces.Discrete(32),
                "dealers_card": spaces.Discrete(11),
    ...
    def _get_obs(self):
        return {
            "players_sum": sum_hand(self.player),
            "dealers_card": self.dealer[0],
            "usable_ace": usable_ace(self.player)
        }
    ...
    def reset(
        ...
        obs = self._get_obs()
        dealer_card_value = obs["dealers_card"]
    ...
    def render(self):
        ...
        obs = self._get_obs()
        player_sum = obs['players_sum']
        dealer_card_value = obs['dealers_card']
        usable_ace = obs['usable_ace']
    ...
    def get_observation_schema(self):
        ...
        'players_sum':
            {
                'description': 'The sum of the player\'s cards',
                'type': 'int',
                'size': 1,
            },
        'dealers_card':
            {
                'description': 'The value of the dealer\'s showing card',
                'type': 'int',
                'size': 1,
            },
        'usable_ace':
            {
                'description': 'Whether the player has a usable ace',
                'type': 'int',
                'size': 1,
            }
        }
    ...
    def get_goal_description(self):
        return """Blackjack is a card game where the goal is to beat the dealer by obtaining cards that sum to closer to 21 (without going over 21) than the dealers cards.
